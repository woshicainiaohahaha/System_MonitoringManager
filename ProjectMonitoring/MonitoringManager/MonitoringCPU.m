...
@implementation MonitoringCPU

+ (void)checkCPUSpeed:(void(^)(NSArray<NSString *> *use))used {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        float cpuUsage = [self cpuUsage];
        float memoryUsage = [self memoryUsage];
        
        used(@[[NSString stringWithFormat:@"%.2f%%",cpuUsage?cpuUsage:0.f],[NSString stringWithFormat:@"%.1f",memoryUsage?memoryUsage:0.f]]);
    });
}
